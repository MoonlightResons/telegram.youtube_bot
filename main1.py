...
async def about_info(message: types.Message):
    await bot.send_message(chat_id=message.chat.id, text='Привет, данный бот создан мной (Нурбеком) как первый проект для Ogogo Academy. Он будет тебе полезен для скачивания видео и аудио из YouTube в высоком качестве. Приятного использования!', reply_markup=menu())
    ...
    if message.text.lower() == "🏵 Отмена":
        ...
        if search_results:
            ...
            for video in search_results:
                video_info = f"Название: {video.title}\nАвтор: {video.author}\nСсылка: {video.url}"
                await bot.send_message(
                    chat_id=message.chat.id,
                    text=video_info,
                    reply_markup=make_keyboards(video.url)
                )
        else:
            await bot.send_message(
                chat_id=message.chat.id,
...
async def perform_search(query):
    ...
    for item in search_response['items']:
        ...
        video = Video(title=video_title, url=video_url)
        search_results.append(video)

    return search_results
    # video = Video(title="Video Title", author="Video Author", url="https://www.youtube.com/watch?v=...")
    # search_results.append(video)
...
async def handler_call(call: types.CallbackQuery, state: FSMContext):
    ...
    if call.data.startswith('best_with_audio'):
        ...
        await bot.send_message(chat_id=chat_id, text=f'Вот ваша ссылка на скачивание видео: {download_link}', reply_markup=back())
        await state.finish()
    elif call.data.startswith('best_audio'):
        await bot.delete_message(call.message.chat.id, call.message.message_id)
        video_url = get_url(call.data)
        download_link = get_download_url_best_audio(video_url)
        await bot.send_message(chat_id=chat_id, text=f'Вот ваша ссылка на скачивание аудио: {download_link}', reply_markup=back())
        await state.finish()
